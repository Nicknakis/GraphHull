...
Defines the main `LSM` class with clustering evaluation methods (NMI / ARI)
and a likelihood schedule tuned for label-recovery experiments.
"""
...
class LSM(nn.Module, Spectral_clustering_init):
    def __init__(self, number_of_K, latent_dim, k_extra, sparse_i, sparse_j,
        ...
        # initialization
        Spectral_clustering_init.__init__(self, method='Normalized',
                                          num_of_eig=number_of_K, device=device)
    ...
    def LSM_likelihood_bias_cs(self, epoch, temperature=1, r=1, Poisson=False):
        ...
        if self.phase_1:
            ...
            #  + self.dpp_across_hulls_centroid_gram(self.A_all)
    ...
    def clustering_(self, true_labels):
        # NOTE: This method references `k_labs` and `SEED` from outside the class
        # in the original script. It is preserved exactly for reference and is not
        # invoked by the main training loop.
        with torch.no_grad():
            Z = self.latent_z.detach().cpu().numpy()
            scaler = StandardScaler().fit(Z)
            Z = scaler.transform(Z)
        kmeans = KMeans(n_clusters=k_labs, n_init=50, random_state=SEED, algorithm='lloyd')  # noqa: F821
